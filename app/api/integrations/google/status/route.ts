    ...
    return NextResponse.json(
      {
        ...
        automationEnabled: integration?.automationEnabled === true,
        gmailAutoScanEnabled: integration?.gmailAutoScanEnabled === true,
        calendarAssistEnabled: integration?.calendarAssistEnabled === true,
        autoCommitCalendar: integration?.autoCommitCalendar === true,
        calendarAutoCommitAvailable: process.env.LINGT_AUTOCOMMIT_CALENDAR === 'true',
        gmailReady: Boolean(integration?.scope?.includes('gmail.readonly')),
        calendarReady: Boolean(
          integration?.scope?.includes('calendar.events') || integration?.scope?.includes('calendar.readonly'),
        ),
        oauthConfigured: hasGoogleOAuthConfig(),
